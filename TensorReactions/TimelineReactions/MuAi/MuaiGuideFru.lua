	...
	[29] = 
	{
		
		{
			data = 
			{
				...
				name = "清标点",
				timelineIndex = 29,
				uuid = "2d86a6b7-2b1e-8f4b-a89b-06feb22a6600",
				version = 2,
			},
		},
	},
			...
			data = 
			{
				actions = 
				{
					
					{
						...
						{
							aType = "Lua",
							actionLua = "local drawTime = 13000\nlocal M = MuAiGuide\nlocal center = { x = 100, y = 0, z = 100 }\nlocal runRadius = 16.1\nlocal arcRadius = 18\nlocal arcStep = math.pi / 12 -- 15 degrees per segment\n\nlocal function consumeWithError(reason)\n    M.Debug(\"[DD跑圈] 跳过绘制：\" .. reason)\n    self.used = true\nend\n\nif data.MuAiGd_DiamondDustKnockBackPos == nil then\n    consumeWithError(\"缺少击退站位\")\n    return\nend\n\nlocal player = TensorCore.mGetPlayer()\nif player == nil or player.pos == nil then\n    consumeWithError(\"未找到玩家实体\")\n    return\nend\n\nlocal function getEightDirection(pos)\n    local heading = M.SetHeading2Pi(TensorCore.getHeadingToTarget(center, pos))\n    return (math.floor((heading + math.pi / 8) / (math.pi / 4)) % 8) + 1\nend\n\n-- Local logs: the real light clone is ModelID 17824 at radius 9.959-10.024.\n-- Later ice actors reuse ContentID 13554 with ModelID 9020 at radius 16.\nlocal lightBoss\nlocal lightBossRadius\nlocal candidateDirection\nlocal ambiguousDirection = false\nfor _, ent in pairs(TensorCore.entityList(\"contentid=13554\") or {}) do\n    if ent ~= nil and ent.id ~= nil and ent.pos ~= nil and Argus.getEntityModel(ent.id) == 17824 then\n        local radius = TensorCore.getDistance2d(center, ent.pos)\n        if radius >= 9.5 and radius <= 10.5 then\n            local direction = getEightDirection(ent.pos)\n            if candidateDirection == nil then\n                candidateDirection = direction\n            elseif candidateDirection ~= direction then\n                ambiguousDirection = true\n            end\n            if lightBoss == nil or math.abs(radius - 10) < math.abs(lightBossRadius - 10) then\n                lightBoss = ent\n                lightBossRadius = radius\n            end\n        end\n    end\nend\n\nif lightBoss == nil then\n    consumeWithError(\"未找到 ModelID 17824 的光分身\")\n    return\nend\nif ambiguousDirection then\n    consumeWithError(\"光分身候选方向不唯一\")\n    return\nend\n\nlocal selfDirection = getEightDirection(data.MuAiGd_DiamondDustKnockBackPos)\nlocal bossDirection = getEightDirection(lightBoss.pos)\nlocal delta = (selfDirection - bossDirection) % 8\nlocal runSign\nlocal runSteps\nlocal message\n\n-- Preserve the original DDRunType strategy with integer octant differences.\nif delta == 1 or delta == 5 then\n    if M.Config.FruCfg.DDRunType == 1 then\n        runSign = 1\n        runSteps = 3\n        message = \"反跑135度！\"\n    elseif delta == 1 then\n        runSign = 1\n        runSteps = 2\n        message = \"反跑90度！\"\n    else\n        runSign = -1\n        runSteps = 2\n        message = \"正跑90度！\"\n    end\nelseif delta == 2 or delta == 6 then\n    runSign = -1\n    runSteps = 2\n    message = \"正跑90度！\"\nelseif delta == 3 or delta == 7 then\n    runSign = -1\n    runSteps = 3\n    message = \"正跑135度！\"\nelseif delta == 0 or delta == 4 then\n    runSign = -1\n    runSteps = 4\n    message = \"正跑180度！\"\nend\n\nif runSign == nil or runSteps == nil then\n    consumeWithError(\"无法解算八方向差值：\" .. tostring(delta))\n    return\nend\n\nlocal function drawArcArrow(startHeading, arcHeading, timeout)\n    local green = GUI:ColorConvertFloat4ToU32(0, 1, 0, 0.5)\n    local drawer = TensorCore.getStaticDrawer(green, 3)\n    local segmentCount = math.max(1, math.floor(math.abs(arcHeading) / arcStep + 0.5))\n    local sign = arcHeading > 0 and 1 or -1\n    local lastPos = TensorCore.getPosInDirection(center, startHeading, arcRadius)\n\n    for i = 1, segmentCount - 1 do\n        local nextHeading = startHeading + sign * arcStep * i\n        local nextPos = TensorCore.getPosInDirection(center, nextHeading, arcRadius)\n        drawer:addTimedLine(\n            timeout,\n            lastPos.x, player.pos.y, lastPos.z,\n            nextPos.x, player.pos.y, nextPos.z,\n            0.55, nil, 0)\n        lastPos = nextPos\n    end\n\n    local finalPos = TensorCore.getPosInDirection(center, startHeading + arcHeading, arcRadius)\n    local finalHeading = TensorCore.getHeadingToTarget(lastPos, finalPos)\n    local finalLength = TensorCore.getDistance2d(lastPos, finalPos)\n    drawer:addTimedArrow(\n        timeout,\n        lastPos.x, player.pos.y, lastPos.z,\n        finalHeading,\n        math.max(finalLength - 1.2, 0.2), 0.55, 1.2, 1.2, 0)\nend\n\nlocal selfHeading = (selfDirection - 1) * math.pi / 4\nlocal arcHeading = runSign * runSteps * math.pi / 4\nlocal guideHeading = M.SetHeading2Pi(selfHeading + arcHeading)\nlocal endPos = TensorCore.getPosInDirection(center, guideHeading, runRadius)\nlocal arcSegments = math.floor(math.abs(arcHeading) / arcStep + 0.5)\n\nM.Info(message, true)\nM.Debug(string.format(\n    \"[DD跑圈] selfDir=%d bossDir=%d delta=%d bossID=%s bossR=%.3f sign=%d steps=%d arcShapes=%d\",\n    selfDirection,\n    bossDirection,\n    delta,\n    tostring(lightBoss.id),\n    lightBossRadius,\n    runSign,\n    runSteps,\n    arcSegments))\n\nM.DirectTo(endPos.x, endPos.z, drawTime)\ndrawArcArrow(selfHeading, arcHeading, drawTime)\nself.used = true\n",
							gVar = "ACR_TensorRequiem3_CD",
			...
			data = 
			{
				...
				conditions = 
				{
					
					{
						...
						{
							category = "Lua",
							conditionLua = "return MuAiGuide.Config.Main.AnyOneReactionOn == false",
							name = "检查设置",
			...
			data = 
			{
				actions = 
				{
					
					{
						...
						{
							aType = "Lua",
							actionLua = "MuAiGuide.Info(\"看向场外。\", true)\nself.used = true",
							gVar = "ACR_RikuWHM3_CD",
							uuid = "af83de3d-2a10-2b37-9cd6-eef65d1696a5",
							version = 2.1,
						},
					},
				},
				...
				timelineIndex = 135,
				timerOffset = -2,
				uuid = "c8e1a736-fc3b-27ee-a6b9-31dabc6b026e",
				version = 2,
			},
			inheritedIndex = 3,
		},
			...
			data = 
			{
				actions = 
				{
					
					{
						...
						{
							aType = "Lua",
							actionLua = "local M = MuAiGuide\n\nif data.MuAiGd_P3_2_ActGroupFinish == true then\n    self.used = true\n    return\nend\nif M.Party == nil then\n    self.used = true\n    return\nend\n\nlocal markerToJob = {\n    [1] = \"MT\",\n    [2] = \"ST\",\n    [3] = \"H1\",\n    [4] = \"H2\",\n    [5] = \"D4\",\n    [6] = \"D1\",\n    [7] = \"D2\",\n    [8] = \"D3\",\n    [9] = \"D4\",\n}\nlocal nextState = {}\nlocal seenAssignments = {}\nlocal selfJob\nlocal selfIsLeft\nlocal playerCount = 0\n\nfor job, party in pairs(M.Party) do\n    local curPlayer = TensorCore.mGetEntity(party.id)\n    local marker = curPlayer and tonumber(curPlayer.marker) or nil\n    local assignedJob = marker and markerToJob[marker] or nil\n    if assignedJob == nil or seenAssignments[assignedJob] then\n        self.used = true\n        return\n    end\n    seenAssignments[assignedJob] = true\n\n    local waterBuff = TensorCore.getBuff(party.id, 2461)\n    local buffType = 4\n    if waterBuff then\n        if waterBuff.duration > 30 then\n            buffType = 3\n        elseif waterBuff.duration > 20 then\n            buffType = 2\n        else\n            buffType = 1\n        end\n    end\n\n    local isLeft = marker <= 4\n    nextState[job] = { IsLeft = isLeft, Type = buffType }\n    playerCount = playerCount + 1\n    if job == M.SelfPos then\n        selfJob = assignedJob\n        selfIsLeft = isLeft\n    end\nend\n\nif playerCount ~= 8 or selfJob == nil then\n    self.used = true\n    return\nend\n\ndata.MuAiGd_P3_2_PlayerBuffType = nextState\ndata.MuAiGd_P3_2_SelfJob = selfJob\nlocal guideIssued\nif selfIsLeft then\n    M.Info(\"检测到攻击标记，去左侧。\")\n    guideIssued = M.DirectTo(93, 100, 4000)\nelse\n    M.Info(\"检测到非攻击标记，去右侧。\")\n    guideIssued = M.DirectTo(107, 100, 4000)\nend\nif guideIssued then\n    data.MuAiGd_P3_2_ActGroupFinish = true\nend\nself.used = true\n",
							conditions = 
							{
			...
			data = 
			{
				actions = 
				{
					
					{
						...
						{
							aType = "Lua",
							actionLua = "if MuAiGuide.SelfPos == \"MT\" and MuAiGuide.Config.FruCfg.P3DarkestDanceTaker == 1\n        or MuAiGuide.SelfPos == \"ST\" and MuAiGuide.Config.FruCfg.P3DarkestDanceTaker == 2\nthen\n    local pos = TensorCore.getPosInDirection({ x = 100, y = 0, z = 100 }, data.MuAiGd_P3_2_MTHeading, 19)\n    MuAiGuide.Info(\"远离引导跳跃。\")\n    MuAiGuide.DirectTo(pos.x, pos.z, 2000, 0.5)\nend\nself.used = true\n ",
							gVar = "ACR_TensorRequiem3_CD",
				...
				conditions = 
				{
					...
					
					{
						data = 
						{
							...
							category = "Self",
							name = "有蓝BUFF",
							uuid = "bb517077-607c-691b-b24a-367cf672600b",
							version = 3,
						},
					},
			...
			data = 
			{
				...
				conditions = 
				{
					
					{
						...
						{
							category = "Lua",
							conditionLua = "return data.MuAiGd_P4_Flags == nil or data.MuAiGd_P4_Flags.EndGuide == nil or data.MuAiGd_P4_Flags.EndGuide == false",
							uuid = "0a1785e8-944f-5948-962b-338d0e244cff",
							version = 3,
						},
					},
				},
				...
				timerOffset = -1,
				timerStartOffset = -1,
				uuid = "ee0a67c2-d69e-daaf-99ac-f0ec949b0829",
				version = 2,
			},
			inheritedIndex = 3,
		},
			...
			data = 
			{
				...
				conditions = 
				{
					
					{
						...
						{
							category = "Lua",
							conditionLua = "if MuAiGuide.Config.FruCfg.CrystallizeTimeBuffType == 2\n        and ((data.MuAiGd_P4_SelfGuideType == 5 and data.MuAiGd_P4_GroundType == 2)\n        or (data.MuAiGd_P4_SelfGuideType == 6 and data.MuAiGd_P4_GroundType == 1)) then\n    local ice = TensorCore.getBuff(MuAiGuide.GetPlayer().id, 2462)\n    if ice and ice.duration < 2.5 then\n        return true\n\telse\n\t\treturn false\n    end\nelse\n    return false\nend",
	...
	[201] = 
	{
		
		{
			data = 
			{
				...
				name = "清标点",
				timelineIndex = 201,
				uuid = "88d0a2f3-c31c-7c8d-bea0-4fcebf3958c3",
				version = 2,
			},
		},
	},
